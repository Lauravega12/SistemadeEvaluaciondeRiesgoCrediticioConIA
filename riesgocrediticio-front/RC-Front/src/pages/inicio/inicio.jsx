...
function Inicio() {
  ...
  useEffect(() => {
    ...
    const fetchAssessment = async () => {
      try {
        const response = await axios.get("http://localhost:8080/api/user/assessment", {
          headers: {
  ...
  if (loading) {
    return (
      <>
        <Navbar />
        <p style={{ textAlign: "center", marginTop: "2rem" }}>Cargando información...</p>
      </>
    );
  }
        ...
        <div className="inicio-Perfil">
          <header className="inicio-Perfil-header">
            ...
            {!assessment?.riskLevel && (
              <p>Aún no se ha podido obtener la descripción del nivel de riesgo.</p>
            )}
          </header>
          ...
          <div className="tags">
            <a href="#">
              {assessment?.age ? `${assessment.age} años` : "Edad no disponible"}
            </a>
            <a href="#">
              {assessment?.income ? `Ingreso: $${assessment.income}` : "Ingreso no disponible"}
            </a>
            <a href="#">
              {assessment?.debt ? `Deuda: $${assessment.debt}` : "Deuda no disponible"}
            </a>
          </div>
        </div>
